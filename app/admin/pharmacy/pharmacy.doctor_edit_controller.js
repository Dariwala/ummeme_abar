// blank controller
pharmacy.controller('PharmacyDoctorEditController', PharmacyDoctorEditController);

function PharmacyDoctorEditController($scope, $http) {

    var pharmacy_doctor_id = document.getElementsByName('pharmacy_doctor_id')[0].value;

    console.log(pharmacy_doctor_id);

    $http
        .get(window.location.origin+"/medical-department/api/pharmacy/get-selected-department/" + pharmacy_doctor_id, {
        ...
        $http
        .get(window.location.origin+"/medical-specialist/api/pharmacy/get-selected-medical-specialist/" + pharmacy_doctor_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
