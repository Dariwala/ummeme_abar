// blank controller
optical.controller('OpticalProductEditController', OpticalProductEditController);

function OpticalProductEditController($scope, $http) {



    var optical_product_id = document.getElementsByName('optical_product_id')[0].value;
    

    $http
        .get(window.location.origin+"/product-category/api/optical/get-selected-product-category/" + optical_product_id, {
        ...
        $http
        .get(window.location.origin+"/product-category/api/optical/get-selected-product-subcategory/" + optical_product_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
