// blank controller
optical.controller('OpticalDoctorEditController', OpticalDoctorEditController);

function OpticalDoctorEditController($scope, $http) {

    var optical_doctor_id = document.getElementsByName('optical_doctor_id')[0].value;

    console.log(optical_doctor_id);

    $http
        .get(window.location.origin+"/medical-department/api/optical/get-selected-department/" + optical_doctor_id, {
        ...
        $http
        .get(window.location.origin+"/medical-specialist/api/optical/get-selected-medical-specialist/" + optical_doctor_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
